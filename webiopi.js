...
WebIOPi.prototype.createMySlider = function(name,functionOutputName,myDataList,min,val,max,steep,macroFunction) {
	var slider = $('<input type="range">');
	if(functionOutputName!=undefined){
	    slider.attr("oninput",functionOutputName+'(value)');
	}
	if(myDataList!=undefined){
	    slider.attr("list",myDataList);
	}
	slider.attr("min",min);
	slider.attr("max",max);
	slider.attr("step",steep);
	slider.attr("id",name);
	slider.attr("value",val);
	slider.bind("change",macroFunction);
	return slider;
}

WebIOPi.prototype.createLabel = function(element,text){
    var label=$('<label>'+text+'</label>');
    label.attr("for",element);
    return label;
    }

WebIOPi.prototype.createOutput = function(id,element,defaultText){
    var output;
    if(defaultText!=undefined){
        output=$('<output>'+defaultText+'</output>');
    }else{
    output=$('<output></output>');
    }
    output.attr("for",element);
    output.attr("id",id);
    return output;
    }
